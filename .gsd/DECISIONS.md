...
- "Single compose file set for Docker and Podman — no forks; Podman-specific settings via override files (docker-compose.podman.yml)"
- "SELinux :Z/:z volume labels in separate override files (docker-compose.podman-selinux.yml) — never in base compose files"
