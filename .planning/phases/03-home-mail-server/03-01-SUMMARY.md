...
tags: [stalwart, maddy, postfix, dovecot, docker-compose, imap, smtp, lmtp, lmdb]

# Dependency graph
requires:
  - phase: 01-transport-layer
    provides: WireGuard tunnel (10.8.0.x network) for cloud relay to home device communication
  - phase: 02-cloud-relay
    provides: SMTP forwarding to home device via WireGuard (10.8.0.2:25)
provides:
...
affects: [03-02-user-management, 03-03-spam-filtering, 04-dns-auth]

# Tech tracking
tech-stack:
  added:
    - stalwartlabs/stalwart:0.15.4 (all-in-one mail server, IMAP4rev2, JMAP, CalDAV)
    - foxcpp/maddy:0.8.2 (Go-based all-in-one mail server)
    - Postfix 3.7+ (MTA on Alpine 3.21)
    - Dovecot 2.3+ (IMAP/LMTP server)
    - LMDB (Postfix map format, replaces deprecated BerkleyDB)
  patterns:
    - Docker compose profiles for runtime component selection
    - Self-signed TLS within WireGuard tunnel (traffic already encrypted)
    - Virtual mailbox domains (no system users, UID/GID 5000)
    - LMTP for internal Postfix-to-Dovecot delivery
    - SMTP for cloud relay to home mail server delivery
