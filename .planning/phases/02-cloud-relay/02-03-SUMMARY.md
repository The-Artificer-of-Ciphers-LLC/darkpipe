...
tags: [testing, ephemeral, docker, optimization, integration]
dependency_graph:
  requires:
    - phase: 02-cloud-relay
      plans: [02-01, 02-02]
      components: [cloud-relay/relay/smtp, cloud-relay/relay/forward, cloud-relay/relay/config]
    - phase: 01-transport-layer
      plans: [01-02]
      components: [transport/mtls/testutil]
  provides:
    - cloud-relay/relay/ephemeral (ephemeral storage verification)
    - cloud-relay/relay/forward/mock.go (mock forwarder for testing)
    - cloud-relay/tests (integration test suite)
    - .dockerignore (optimized build context)
  affects:
    - cloud-relay/Dockerfile (optimized for <50MB image size)
    - cloud-relay/docker-compose.yml (256MB memory limit)
tech_stack:
  added:
    - stdlib testing (no external test frameworks)
  patterns:
    - Mock forwarder pattern for isolated SMTP session testing
    - Integration test helper for server lifecycle management
    - Ephemeral storage verification with violation classification
key_files:
  created:
    - cloud-relay/relay/ephemeral/verify.go (ephemeral storage verification)
    - cloud-relay/relay/ephemeral/verify_test.go (verification tests)
    - cloud-relay/relay/forward/mock.go (mock forwarder for testing)
    - cloud-relay/relay/config/config_test.go (configuration tests)
    - cloud-relay/relay/forward/mtls_test.go (mTLS forwarder tests)
    - cloud-relay/relay/forward/wireguard_test.go (WireGuard forwarder tests)
    - cloud-relay/relay/smtp/server_test.go (SMTP backend tests)
    - cloud-relay/relay/smtp/session_test.go (SMTP session tests)
    - cloud-relay/tests/integration_test.go (end-to-end integration tests)
    - .dockerignore (build context optimization)
  modified:
    - cloud-relay/Dockerfile (Alpine 3.21 optimization, labels, Postfix 3.9 pinning)
    - cloud-relay/docker-compose.yml (256MB memory limit, ephemeral check interval)
decisions:
  - "Ephemeral verification scans Postfix queue directories (incoming, active, deferred, hold, corrupt) and ignores control files (pid, master.lock, dotfiles)"
  - "MockForwarder exported in forward/mock.go for use across test packages rather than internal test file"
  - "Integration tests use net.Listen to get actual port before starting server (avoids 127.0.0.1:0 addressing issue)"
  - "Large message test uses properly formatted SMTP lines (70 chars/line) to avoid SMTP library line length limits"
  - "Dockerfile optimization: Alpine 3.21 base, stripped Go binary (-ldflags='-s -w'), Postfix version pinning, .dockerignore excludes .git/.planning/docs/tests"
  - "Docker compose 256MB memory limit enforced via deploy.resources.limits per UX-02 requirement"
  - "Config tests cannot use t.Parallel() with t.Setenv() (Go 1.17+ restriction)"
  - "All tests use stdlib testing only (no testify) for consistency with Phase 1 approach"
metrics:
  duration: 542s
  tasks_completed: 2
  files_created: 10
