...
tags: [setup, cli, docker-compose, validation, interactive, go]
dependency_graph:
  requires: []
  provides:
    - darkpipe-setup interactive CLI tool
    - Docker Compose file generation
    - Live DNS and SMTP validation
    - Configuration schema migration framework
  affects:
    - User deployment experience (UX-01)
    - Setup workflow for all platforms
tech_stack:
  added:
    - github.com/AlecAivazis/survey/v2@v2.3.7 (interactive prompts)
    - github.com/pterm/pterm@v0.12.79 (rich terminal output)
    - github.com/spf13/cobra@v1.8.1 (CLI framework)
    - github.com/miekg/dns@v1.1.62 (DNS validation)
    - gopkg.in/yaml.v3@v3.0.1 (YAML serialization)
  patterns:
    - Separate Go module for setup tool (isolated dependencies)
    - Type-safe Docker Compose generation (not string templates)
    - Live validation with user-friendly spinners and warnings
    - Quick vs Advanced mode for tiered UX
    - Upgrade-aware config with version-based migration
key_files:
  created:
    - deploy/setup/cmd/darkpipe-setup/main.go (CLI entrypoint, 393 lines)
    - deploy/setup/pkg/config/config.go (Config struct with YAML)
    - deploy/setup/pkg/validate/dns.go (DNS MX/A/AAAA/SPF/DKIM/DMARC validation)
    - deploy/setup/pkg/validate/smtp.go (SMTP port 25 and banner validation)
    - deploy/setup/pkg/validate/ports.go (Port connectivity checks, IPv6-safe)
    - deploy/setup/pkg/secrets/secrets.go (Docker secrets generation, 0600 perms)
    - deploy/setup/pkg/migrate/migrate.go (Version-based config migration)
    - deploy/setup/pkg/compose/types.go (Docker Compose type definitions)
    - deploy/setup/pkg/compose/templates.go (Service template functions)
    - deploy/setup/pkg/compose/generate.go (Compose file generation)
  modified:
    - deploy/setup/go.mod (added 5 dependencies)
    - deploy/setup/go.sum (dependency checksums)
decisions:
  - title: Quick vs Advanced setup modes
    rationale: UX-01 requires tiered experience - Quick mode asks 3 questions (domain, relay hostname, admin email) and uses opinionated defaults (Stalwart + SnappyMail + builtin calendar). Advanced mode allows full component customization.
    alternatives: Single mode with all questions, or config file only
    chosen: Two-mode approach balances simplicity and control
  - title: Type-safe YAML generation over string templates
    rationale: Using Go structs that marshal to YAML provides compile-time safety, prevents YAML syntax errors, and enables conditional service inclusion logic
    alternatives: Text templates with .env, HEREDOC string concatenation
    chosen: gopkg.in/yaml.v3 with ComposeFile structs
  - title: Non-blocking validation warnings
    rationale: DNS may not be configured yet during setup. Port 25 may be blocked until VPS provisioned. Warnings inform user but allow continuation to generate config first.
    alternatives: Hard-fail on validation errors, skip validation entirely
    chosen: Warn and continue (validate but don't block)
  - title: Separate Go module for setup tool
    rationale: Setup tool has dependencies (survey, pterm, cobra) that mail relay doesn't need. Separate module prevents dependency bloat in core services.
    alternatives: Single shared go.mod, vendor all dependencies
    chosen: deploy/setup has its own go.mod
metrics:
  duration: 389s
  tasks_completed: 2
  files_created: 13
  lines_added: 1621
  commits: 2
  completed_at: 2026-02-14T14:07:04Z
---
