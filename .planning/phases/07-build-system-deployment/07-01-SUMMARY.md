...
tags: [docker, multi-arch, github-actions, cicd, deployment, optimization]
dependency_graph:
  requires: [phase-06-complete]
  provides: [multi-arch-images, github-workflows, docker-secrets, setup-detection]
  affects: [all-deployments]
tech_stack:
  added: [github-actions, docker-buildx, qemu, oci-labels]
  patterns: [multi-stage-builds, docker-secrets, entrypoint-wrappers, workflow-dispatch]
key_files:
  created:
  ...
  modified:
    ...
    - .dockerignore
decisions:
  - Multi-arch support via TARGETARCH build arg (buildx sets automatically)
  - OCI labels on all images (source, version, description, licenses)
  - Netcat health checks (more reliable than process-based checks)
  - Docker secrets via _FILE suffix convention for all sensitive values
  - Setup detection at /config/.darkpipe-configured
  - GHCR as sole registry (no Docker Hub)
  - Pre-built images for two stacks: default (Stalwart+SnappyMail) and conservative (Postfix+Dovecot+Roundcube+Radicale)
  - GitHub Actions cache (type=gha) for layer optimization
  - Conditional setup tool build (only if source exists)
metrics:
