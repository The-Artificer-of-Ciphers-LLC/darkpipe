...
tags: [status-aggregation, web-dashboard, cli, push-monitoring, integration-tests]
dependency-graph:
  requires:
    - Phase 09-01 (health, queue, delivery, cert packages)
    - Phase 09-02 (alert, cert lifecycle packages)
    - Phase 08 (profile server infrastructure)
  provides:
    - Unified status aggregation across all monitoring packages
    - CLI status command with human and JSON output
    - Web dashboard for household members
    - Push-based external monitoring (Healthchecks.io)
    - Docker health checks for all containers
    - Phase 9 integration test suite
  affects:
    - Profile server (new /status and /status/api routes)
    - Docker Compose (health checks, monitoring env vars)
    - Caddy (Basic Auth routes for remote monitoring)
tech-stack:
  added: []
  patterns:
    - Interface-based aggregation for testability
    - Template-based web rendering
    - Dead Man's Switch push monitoring
    - NDJSON for CLI alert storage
    - Auto-refresh via meta tag (no JavaScript)
key-files:
  created:
  ...
  modified:
    - home-device/docker-compose.yml (monitoring env vars, health check update)
    - cloud-relay/docker-compose.yml (relay health check)
    - cloud-relay/caddy/Caddyfile (Basic Auth routes for /health and /status)
decisions:
  - "Status aggregator uses interface-based design (HealthChecker, CertWatcher, DeliveryTracker) for testability"
  - "CLI command shows human-readable by default, --json for scripting, --watch for live monitoring"
  - "Web dashboard added to profile server at /status (already HTTP server, 64MB memory sufficient)"
  - "Auto-refresh via meta tag (30s) instead of JavaScript for simplicity and compatibility"
  - "Push pinger uses Dead Man's Switch pattern: external service alerts when pings stop"
  - "Docker HEALTHCHECK updated to /health/live (liveness check, always up if process alive)"
  - "Caddy Basic Auth protects /health/* and /status/* for remote monitoring access"
  - "Phase test suite follows established pattern from Phase 8 with pass/fail reporting"
metrics:
  duration: 456
  tasks: 2
  commits: 2
  files: 12
  tests: 23
  completed: 2026-02-14T16:39:00Z
---
