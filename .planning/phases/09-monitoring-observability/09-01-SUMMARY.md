...
tags: [monitoring, health-checks, queue-stats, delivery-tracking]
dependency-graph:
  requires: [transport/health, postfix]
  provides: [monitoring/health, monitoring/queue, monitoring/delivery]
  affects: []
tech-stack:
  added: []
  patterns: [ring-buffer, health-check-separation, mock-executor, json-serialization]
key-files:
  created:
  ...
  modified: []
decisions:
  - "Liveness vs Readiness: Liveness always returns 'up' (process alive), readiness performs deep checks (Postfix port 25, IMAP port 993, tunnel health)"
  - "Health check HTTP handlers return application/health+json content type (future Kubernetes compatibility)"
  - "Queue monitor uses postqueue -j JSON output (not text parsing) for reliability"
  - "Stuck message threshold: 24 hours (configurable via QueueStats.StuckThreshold field)"
  - "Delivery tracker uses ring buffer (not database) for 1000 recent entries to avoid I/O overhead"
  - "Ring buffer capacity configurable via MONITOR_DELIVERY_HISTORY env var"
  - "Thread-safe tracker with RWMutex for concurrent log parsing and query access"
  - "Postfix log parser extracts both inbound (relay->home) and outbound (home->world) deliveries"
  - "Status normalization: sent->delivered, deferred->deferred, bounced->bounced, expired->expired"
metrics:
  duration: 388
  tasks: 2
  commits: 2
