...
plan: 02
subsystem: caldav-carddav
tags: [radicale, caldav, carddav, caddy, reverse-proxy, calendar, contacts]

# Dependency graph
requires:
  - phase: 06-01
    provides: Caddy reverse proxy on cloud relay and webmail infrastructure
  - phase: 03-home-mail-server
    provides: Mail server profiles (stalwart, maddy, postfix-dovecot) with user databases
provides:
...
affects: [08-device-setup]

# Tech tracking
tech-stack:
  added:
    - tomsquest/docker-radicale:3.6.0 (CalDAV/CardDAV server)
  patterns:
    - Radicale profile pattern (matches Phase 3 mail server profile selection)
    - Htpasswd bcrypt authentication synchronized from mail server users
    - File-based storage with Radicale .Radicale.props metadata format
    - Well-known URL auto-discovery pattern (RFC 6764)
    - Shared collection pattern with rights file ACLs
...
patterns-established:
  - "CalDAV/CardDAV profiled deployment pattern (radicale profile for non-Stalwart setups)"
  - "Manual user sync pattern (sync-users.sh) as interim solution before Phase 8 automation"
  - "Radicale .Radicale.props JSON metadata for collection properties"
  - "Rights file ACL pattern for calendar/contact sharing"
  - "Phase test suite pattern (test-webmail-groupware.sh covers all success criteria)"
