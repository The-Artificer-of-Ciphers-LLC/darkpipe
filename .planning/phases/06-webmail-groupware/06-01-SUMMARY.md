...
plan: 01
subsystem: webmail
tags: [caddy, roundcube, snappymail, reverse-proxy, imap, webmail]

# Dependency graph
requires:
  - phase: 01-transport-layer
    provides: WireGuard tunnel between cloud relay and home device (10.0.0.0/24 network)
  - phase: 02-cloud-relay
    provides: Cloud relay infrastructure and docker-compose pattern
  - phase: 03-home-mail-server
    provides: Mail server profiles (stalwart, maddy, postfix-dovecot) exposing IMAP/SMTP
provides:
...
affects: [06-02-caldav-carddav, 08-device-setup]

# Tech tracking
tech-stack:
  added:
    - caddy:2-alpine (reverse proxy with auto-HTTPS)
    - roundcube/roundcubemail:1.6.13 (PHP webmail client)
    - djmaze/snappymail:2.38.2 (lightweight webmail client)
  patterns:
    - Webmail profile selection pattern (matches Phase 3 mail server profiles)
    - IMAP passthrough authentication (mail server as source of truth)
    - extra_hosts mail-server:host-gateway for cross-profile compatibility
    - Caddy reverse proxy over WireGuard tunnel for remote access
...
patterns-established:
  - "Webmail selection via Docker compose profiles (docker compose --profile stalwart --profile roundcube up -d)"
  - "Caddy reverse proxy pattern for remote access over WireGuard tunnel"
  - "Port 8080 as standard webmail port on home device"
  - "Self-signed certificate acceptance within WireGuard tunnel (verify_peer: false)"
