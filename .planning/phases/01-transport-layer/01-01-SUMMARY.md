...
plan: 01
subsystem: transport/wireguard
tags: [wireguard, vpn, encryption, nat-traversal, systemd]
dependency_graph:
  requires: []
  provides:
...
tech_stack:
  added:
    - Go 1.25.7
    - WireGuard (via wireguard-tools)
  patterns:
    - Config generation via text/template
    - CLI wrapper pattern for wg commands
    - Systemd service management with overrides
key_files:
  created:
    - go.mod: "Go module definition for github.com/darkpipe/darkpipe"
    - transport/wireguard/keygen/keygen.go: "WireGuard keypair generation via wg CLI"
    - transport/wireguard/keygen/keygen_test.go: "Tests for key generation (skips if wg not available)"
    - transport/wireguard/config/templates.go: "Config generation for cloud hub and home spoke"
    - transport/wireguard/config/templates_test.go: "Tests for config generation and file permissions"
    - deploy/wireguard/cloud-setup.sh: "Cloud hub deployment script (installs, configures, enables firewall)"
    - deploy/wireguard/home-setup.sh: "Home spoke deployment script (installs, configures, verifies keepalive)"
    - deploy/wireguard/systemd/wg-quick@wg0.service.d/override.conf: "Systemd override for auto-restart on failure"
  modified: []
decisions:
  - "Use stdlib text/template for config generation (zero external dependencies)"
  - "Wrap wg CLI rather than implement crypto (leverage official tools)"
  - "Default PersistentKeepalive=25 for NAT traversal without port forwarding"
  - "0600 permissions for all config files to protect private keys"
  - "Systemd auto-restart with 30s delay to prevent rapid failure loops"
  - "Cloud enables IP forwarding and firewall; home does neither (outbound only)"
metrics:
  duration_seconds: 270
  completed_date: "2026-02-09T01:47:14Z"
  tasks_completed: 2
  files_created: 8
  commits: 2
---
