...
tags: [imap, state-tracking, folder-mapping, resume]
dependency_graph:
  requires: []
  provides: [mailmigrate-core, state-tracker, imap-sync, folder-mapper]
  affects: [future-provider-integrations, cli-wizard]
tech_stack:
  added: [go-imap-v2, json-state-persistence]
  patterns: [thread-safe-state, auto-save, skip-and-report, date-preservation]
key_files:
  created:
  ...
  modified:
    ...
    - deploy/setup/go.sum
decisions:
  - State file path: /data/migration-state.json (aligns with container volume pattern)
  - Auto-save every 100 operations to prevent data loss during long migrations
  - Thread-safe state tracking with sync.RWMutex for concurrent access
  - go-imap v2.0.0-beta.8 (latest, v2 API redesign with better concurrency)
  - Folder tracking added to state (bonus: enables folder-level resume in future)
  - Defer fetchCmd.Close() to ensure cleanup on error paths
metrics:
  duration: 422s
  completed: 2026-02-14T22:01:48Z
  tasks: 2
  files: 6
  tests: 34
---
