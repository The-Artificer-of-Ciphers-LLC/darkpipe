...
tags: [qrcode, profile-server, autoconfig, autodiscover, srv-records, caddy, single-use-tokens]
...
requires:
  ...
  - phase: 06-01
    provides: Caddy reverse proxy infrastructure
  - phase: 04-02
    provides: DNS automation tooling
provides:
...
affects: [08-03-webmail-integration]

# Tech tracking
tech-stack:
  added:
    - github.com/skip2/go-qrcode (v0.0.0-20200617195104) - QR code PNG and terminal generation
  patterns:
    - Single-use token pattern with MemoryTokenStore (thread-safe, auto-cleanup)
    - crypto/rand for 256-bit token entropy (base64url encoded)
    - Graceful HTTP server shutdown with context timeout
    - JSON request logging for container environments
    - Caddy handle directives for path-based routing

key-files:
  created:
    - home-device/profiles/pkg/qrcode/token.go - Single-use token store with expiry
    - home-device/profiles/pkg/qrcode/generator.go - QR code PNG and terminal generation
    - home-device/profiles/cmd/profile-server/main.go - HTTP server entry point
    - home-device/profiles/cmd/profile-server/handlers.go - Profile/autoconfig/autodiscover handlers
    - dns/records/srv.go - RFC 6186 SRV record generation
    - dns/validator/srv.go - SRV and CNAME validation
  modified:
    - cloud-relay/caddy/Caddyfile - Added autodiscovery and profile routes
    - dns/records/guide.go - Extended AllRecords with SRV and CNAME
